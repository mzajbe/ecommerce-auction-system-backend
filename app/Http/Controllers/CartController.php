...
{
    // Check if the auction exists and has ended
    ...
    // Ensure the auction is not already in the winner's cart
    $existingCart = Cart::where('user_id', $winningBid->user_id)
        ->where('auction_id', $auctionId)
        ->first();

    if ($existingCart) {
        return response()->json(['message' => 'Auction already in cart'], 409);
    }
    ...
    return response()->json([
        'message' => 'Winning auction added to cart',
        'cart' => $cart,
    ], 200);
}
